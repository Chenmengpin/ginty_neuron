...
# Load annotations
gtf <- read.delim(file.path(dataDir, 'gencode.v2.annotation.gtf'), sep="\t", header=F, comment="#")
gtf <- gtf[which(gtf$V3 == "gene"),]

# extract ensembl IDs
ensembl <- apply(gtf, 1, function(x){
  s <- unlist(strsplit(as.character(x[9]), ";"))[1]
  id <- unlist(strsplit(s, " ", fixed=T))[2]
  id <- unlist(strsplit(id, ".", fixed=T))[1]
  return(id)
})

# match annotations with count data
annot <- data.frame("chr"=gtf$V1, "start"=gtf$V4, "stop"=gtf$V5, row.names=ensembl, stringsAsFactors=FALSE)
annot <- annot[which(rownames(annot) %in% row.names(counts)),]
m <- match(row.names(annot), row.names(counts))
count.annot <- counts[m,]

# RPKM
y = DGEList(counts=count.annot)
y = calcNormFactors(y)
cpm_counts = cpm(y, normalized.lib.sizes=TRUE)
gene.lengths <- annot$stop - annot$start
rpkm_counts <- rpkm(y, gene.length = gene.lengths)
```
